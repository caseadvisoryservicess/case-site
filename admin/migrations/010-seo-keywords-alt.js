...
// слаг обязателен: молчаливый выбор чужого проекта по умолчанию уже приводил
// к запуску миграции не там, где нужно
const slug = process.argv[2];
if (!slug) {
  console.error('Укажите проект: node migrations/010-seo-keywords-alt.js <slug>');
  console.error('Доступные наборы строк: takhtapul, galaba, kibray-dc');
  process.exit(1);
}
...
const DATA = {
  takhtapul: {
    ...
    alts: {
      'aerial-night.jpg': ml('Коммерческое здание Тахтапул 31 в Ташкенте, вид с высоты ночью',
      ...
      'facade-vertical-dusk.jpg': ml('Фасад здания Тахтапул 31 с витражным остеклением, вечерняя подсветка',
      ...
      'facade-night.jpg': ml('Коммерческое здание под офис, клинику или учебный центр в Шайхантахурском районе Ташкента',
  ...
  galaba: {
    ...
    alts: {
      'facade-dusk.jpg': ml('Коммерческое здание Галаба в Ташкенте, белый фасад с латунной сеткой и арочным стилобатом',
      ...
      'render-1.jpg': ml('Здание под клинику, офис или учебный центр в Алмазарском районе Ташкента, у парка Победы',
      ...
      'render-3.jpg': ml('Первый этаж с витринами на первой линии и арками стилобата, помещение под аптеку или кафе',
  ...
  'kibray-dc': {
    ...
    alts: {
      'gate.jpg': ml('Визуализация въездной группы дата-центра на участке в Кибрае, охраняемый периметр и КПП',
      ...
      'hall.jpg': ml('Визуализация машинного зала дата-центра: ряды стоек, холодный коридор, фальшпол',
      ...
      'power.jpg': ml('Визуализация энергоцентра и чиллеров дата-центра на участке 6,5 га в Ташкентской области',
...
// Привязка по имени файла, а не по порядку: у Кибрая миграция 014 добавила
// снимок с границами участка первым, и позиционная раскладка сдвинула бы
// подписи (машинный зал уехал бы на КПП). Файл, которого нет в наборе,
// не трогаем.
const imgs = (cfg.about && cfg.about.images) || [];
imgs.forEach((g) => {
  const want = D.alts[g.file];
  if (want && JSON.stringify(g.alt) !== JSON.stringify(want)) {
    g.alt = want; changes++;
    console.log('✓ подпись', g.file, '->', want.ru.slice(0, 52) + '...');
  }
});
