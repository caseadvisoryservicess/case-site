...
## Вариант Б-1. Всё на вашем домене: caseadvisory.uz/taxtapul + caseadvisory.uz/admin

Если сайт caseadvisory.uz живёт на **VPS с nginx** — панель ставится рядом,
и всё выглядит так, как вы хотите: лендинг в подпапке, вход в панель — кнопка
«Войти» в шапке лендинга.

1. Установите панель по «Варианту Б» (git clone + `install-vps.sh`) — она
   поднимется на локальном порту 3000.
2. В конфиг nginx вашего сайта добавьте два блока из
   `deploy/nginx.conf.example`: `/taxtapul/` (статика лендинга) и `/admin/`
   (прокси на панель). Перезагрузите nginx.
3. Готово: лендинг — `caseadvisory.uz/taxtapul`, панель — `caseadvisory.uz/admin`.
   Кнопка «Войти» на лендинге уже ведёт на `/admin/` (меняется в панели:
   Основное → «Кнопка „Войти“ на лендинге»).

Каждый новый лендинг из панели = ещё один location-блок в nginx по образцу
(или просто пользуйтесь адресами вида `caseadvisory.uz/admin/p/slug/`,
они работают без настройки).

> Если caseadvisory.uz — на **обычном виртуальном хостинге без Node.js**
> (только PHP/статика): лендинг выкладывайте туда zip-архивом из панели в
> папку `/taxtapul`, а саму панель разместите на Render (Вариант А) — тогда
> в поле «Кнопка „Войти“» впишите её адрес, например
> `https://sfb-panel.onrender.com`.
