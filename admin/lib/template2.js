...
.case-mark i{-webkit-mask:url(${prefix}assets/${caseLogo}) center/contain no-repeat;mask:url(${prefix}assets/${caseLogo}) center/contain no-repeat}`;
  ...
  const counters = [
    ...
    // загрузчик Метрики в текущем виде: id в адресе tag.js и защита от
    // повторной вставки счётчика (проверка уже подключённых скриптов)
    metrica ? `<script>(function(m,e,t,r,i,k,a){m[i]=m[i]||function(){(m[i].a=m[i].a||[]).push(arguments)};m[i].l=1*new Date();for(var j=0;j<e.scripts.length;j++){if(e.scripts[j].src===r){return}}k=e.createElement(t),a=e.getElementsByTagName(t)[0],k.async=1,k.src=r,a.parentNode.insertBefore(k,a)})(window,document,"script","https://mc.yandex.ru/metrika/tag.js?id=${esc(metrica)}","ym");ym(${esc(metrica)},"init",{ssr:true,webvisor:true,clickmap:true,accurateTrackBounce:true,trackLinks:true});</script><noscript><div><img src="https://mc.yandex.ru/watch/${esc(metrica)}" style="position:absolute;left:-9999px" alt=""></div></noscript>` : ''
  ].filter(Boolean).join('\n');
