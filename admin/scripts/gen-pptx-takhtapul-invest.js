...
// ════════════════════════════════════════════════════════════════
{
  ...
  const sc = RMAX / MAXM;
  // Если владелец положил широкий обзор из приложения, показываем его вместо
  // нашей схемы: настоящая карта с улицами читается лучше точек на белом.
  const WIDE = 'caseos-map-wide.jpg';
  if (has(WIDE)) {
    s.addImage({ path: img(WIDE), x: L, y: 1.5, w: 7.5, h: 5.25, sizing: { type: 'cover', w: 7.5, h: 5.25 } });
  } else {
  ...
  leg.forEach((g, i) => {
    const y = 2.1 + i * 0.62;
    if (!has(WIDE)) s.addShape('ellipse', { x: rx + 0.02, y: y + 0.09, w: 0.15, h: 0.15, fill: { color: g[0] }, line: { width: 0 } });
    txt(s, g[1], { x: rx + (has(WIDE) ? 0 : 0.3), y, w: rw - 0.9, h: 0.5, fontSize: 10.5, color: C.ink });
  ...
  txt(s, 'Всего ' + GEO.total + ' объектов в радиусе 3 км. Аптеки и махаллинские центры не показываем: ни офису, ни клинике они не конкуренты.',
    { x: rx, y: 6.1, w: rw, h: 0.7, fontSize: 9.5, color: C.mut });
}
...
// ════════════════════════════════════════════════════════════════
const MAP_DEFS = [
  ['caseos-heat-bc.jpg', 'Зоны пригодности: офис', 'Зелёным отмечены зоны, где людей много, а офисов мало. Наша точка попадает в такую зону и получает лучший из трёх баллов - 78 из 100. Но офисный спрос города стягивается восточнее, в деловые коридоры, поэтому сильный сценарий здесь один: здание целиком под одну компанию.'],
  ['caseos-heat-med.jpg', 'Зоны пригодности: клиника', 'Красное - насыщение: медицинских центров вокруг много, и формально это опускает балл до 55. На практике для клиники соседство работает наоборот: рядом стационары, роддом и диспансеры, а значит поток направлений и район, который люди уже знают как место, куда едут лечиться.'],
  ['caseos-heat-edu.jpg', 'Зоны пригодности: образование', 'Учебных заведений вокруг тоже много, и балл снова 55. Но 82 объекта из 103 в ближней зоне - школы и детские сады, то есть аудитория, а не соперники. Коммерческих курсов рядом почти нет: спрос сформирован, предложения нет.']
];
