...
const MAP_DEFS = [
  { file: 'caseos-map.jpg', h: 'Окружение объекта на карте',
    c: 'Подложка и слои - экспорт приложения CASE OS Geo Analytics (OpenStreetMap, медицина по OSM и clinics.uz, бизнес-центры). Кольцо - радиус 1 км от объекта, тот самый, по которому модель считает балл: NEXUS (358 м) внутри, «Бизнес центр» (891 м) и Alpha (949 м) на границе. В легенде - состав медицины в этом радиусе по направлениям приложения.' },
  { file: 'caseos-heat-bc.jpg', h: 'Зоны пригодности: офис',
    c: 'Модель зон пригодности приложения, сетка 400 м, 2 715 ячеек. Зелёное - людей много, конкурентов мало; красное - наоборот. Балл площадки 78 из 100 при медиане города 83.' },
  { file: 'caseos-heat-med.jpg', h: 'Зоны пригодности: клиника',
    c: 'Та же модель для медицинского формата. Балл площадки 55 из 100 при медиане города 55: штраф за 33 профильных соседа в километре.' },
  { file: 'caseos-heat-edu.jpg', h: 'Зоны пригодности: образование',
    c: 'Та же модель для учебного формата. Балл площадки 55 из 100 при медиане города 55: в конкуренты попали 19 объектов слоя, включая школы и детские сады.' }
];
...
<div class="pg">
  ...
  <div style="display:flex;gap:8mm;flex:1;min-height:0">
    ...
    <div style="flex:1.1;display:flex;flex-direction:column;gap:5mm;min-height:0">
      ...
      <div class="inote">Источник: карточка точки в приложении CASE OS Geo Analytics. Население - сетка Kontur H3, откалиброванная на официальное население района. Медицина по OSM и clinics.uz; профильными конкурентами клиники приложение считает больницы, поликлиники, частные центры, врачебные практики и лаборатории, стоматологии и узкие профили в этот счёт не входят. В счёте бизнес-центров приложение учитывает и сам Тахтапул, поэтому в 1 км стоит 4, а чужих БЦ три: NEXUS 358 м, «Бизнес центр» 891 м, Alpha 949 м.</div>
    </div>
  </div>
  ${foot()}
</div>
  ...
  <div class="itwo">
    ...
    <div style="display:flex;flex-direction:column;gap:5mm;min-height:0">
      <div class="icard">
        ...
        <table class="itab" style="background:transparent;margin-top:2mm">
        ...
        <div class="inote" style="margin-top:2.5mm">Слой образования приложения, ${gnum(EDU_LAYER.city)} объектов по городу (OpenStreetMap, загружено 07.08.2026). Состав по ближней зоне вокруг объекта: ${EDU_NEAR_TOTAL} объектов, из них коммерческий учебный центр один.</div>
      </div>
      <div class="icard acc">
        <div class="ih3">Как читать эти числа</div>
        ${ul(['Медицина, образование и бизнес-центры - из слоёв приложения CASE OS, то есть ровно те данные, по которым посчитан балл площадки.',
              `Модель насчитала образованию ${OS.edu.comp} конкурентов в километре, но считает конкурентами весь слой: школы, детские сады, колледжи и вузы. Для коммерческих курсов это аудитория, а не соперники.`,
              `Слой OSM почти не видит коммерческие курсы: во всём городе у него 12 записей этой категории. Наш ручной срез по 2ГИС, Яндекс.Картам, Golden Pages и Yellow Pages нашёл рядом ${EDU.centres.length}: ${EDU.centres.map(([n]) => esc(n)).join(', ')}.`,
...
<div class="pg">
  ...
  <div class="inote" style="margin-top:5mm">Модель зон пригодности CASE OS: 0,55 x спрос + 0,45 x (1 - конкуренты / эталон), спрос = жители 1 км / ${gnum(OS.benchPop)}, эталон конкуренции ${OS.bench.bc} для всех трёх форматов - поэтому баллы сопоставимы напрямую. Жителей ${gnum(OS.pop[1000])}, спрос упирается в потолок у всех форматов, значит балл задаёт только конкуренция. Приоритет CASE отличается от порядка баллов сознательно: модель штрафует за любое соседство, а в медицине и образовании соседство работает на приток - у клиники это больницы и роддом, у курсов школы и детские сады.</div>
  ${foot()}
</div>
...
<div class="pg">
  ...
  <div class="inote" style="margin-top:5mm">Источники: слои приложения CASE OS Geo Analytics - медицина (OSM и clinics.uz, ${gnum(1530)} объектов), образование (OSM, ${gnum(EDU_LAYER.city)} объектов, загружено 07.08.2026), бизнес-центры (${gnum(151)}), сетка населения Kontur, откалиброванная на официальное население района. Коммерческие учебные центры дополнены ручным срезом по 2ГИС, Яндекс.Картам, Golden Pages и Yellow Pages. Геоаналитика описывает окружение и не заменяет финансовую модель: доходность формата считается отдельно.</div>
  ${foot()}
</div>
`;
}
