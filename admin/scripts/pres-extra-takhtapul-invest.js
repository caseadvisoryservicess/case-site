...
//
// ВАЖНО про 70 против 78. Раньше в документе стояло 78 - это число из панели
// «модель зон пригодности», где эталон конкуренции был выставлен вручную в 8.
// В карточке точки работает встроенный эталон приложения: для БЦ он 6, для
// клиники 8. Пересчёт сходится ровно:
//   БЦ:      0,55 x min(1, 32375/25000) + 0,45 x max(0, 1 - 4/6)  = 70
//   клиника: 0,55 x min(1, 32375/25000) + 0,45 x max(0, 1 - 33/8) = 55
// Берём числа карточки, они каноничные. Медианы города не приводим: они были
// посчитаны при эталоне 8 и с эталоном 6 несопоставимы.
//
// Счёт бизнес-центров в карточке включает сам объект (в списке ближайших он
// стоит первым, «(2 м)»), поэтому в 1 км там 4, а чужих БЦ три.
const OS = {
  metro: { n: 'Бадамзар', d: 1791 },
  benchPop: 25000,
  bench: { bc: 6, med: 8 },
  bc: { score: 70, comp: 4 },
  med: { score: 55, comp: 33 },
...
${dividerPage(ctx, 'geo', `<div style="height:100%;padding:18mm 12mm;display:flex;flex-direction:column;justify-content:center;gap:5mm">
  ...
  ${[[`${OS.bc.score}`, 'Офис, бизнес-центр', `${OS.bc.comp} конкурента в 1 км при эталоне ${OS.bench.bc}`],
     [`${OS.med.score}`, 'Клиника, медцентр', `${OS.med.comp} конкурента в 1 км при эталоне ${OS.bench.med}`]]
  ...
  <div style="font-size:8pt;color:rgba(255,255,255,.5);line-height:1.5">Жителей в радиусе 1 км - ${gnum(OS.pop[1000])}. Спрос упирается в верхнюю границу модели, поэтому разницу между форматами задаёт только конкуренция.</div>
</div>`)}
...
<div class="pg">
  ...
  <table class="itab">
    ...
    <tr>
      ...
      <td>Школа на той же улице и ${gnum(OS.pop[1000])} жителей в километре</td>
    </tr>
    <tr>
  ...
  <div class="inote" style="margin-top:5mm">Быстрый скоринг приложения CASE OS: 0,55 x спрос + 0,45 x (1 - конкуренты / эталон), спрос = жители 1 км / ${gnum(OS.benchPop)}, эталон конкуренции ${OS.bench.bc} для офиса и ${OS.bench.med} для клиники. Жителей ${gnum(OS.pop[1000])}, спрос упирается в потолок у обоих форматов, поэтому балл задаёт только конкуренция. Приоритет CASE отличается от порядка баллов сознательно: модель штрафует за любое соседство, а в медицине соседство работает на приток. Образовательного формата в модели нет.</div>
  ${foot()}
</div>
