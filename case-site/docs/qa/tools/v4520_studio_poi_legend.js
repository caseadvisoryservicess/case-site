...
(async () => {
  ...
  /* ===== Проекты портфеля: с v4.73.1 их в студии нет =====
     Жалоба v4.52: «лишние оранжевые точки на карте» - это были наши проекты из портфеля.
     В v4.73.1 владелец убрал проекты CASE из студии целиком (ею пользуются клиенты):
     слоя, галочки и блока легенды больше нет, на карте только точка анализа. */
  const pf = await pg.evaluate(async () => {
    window.postMessage({ source: 'asaas-os-v4', type: 'asaas-geo-context', context: { lang: 'ru', editable: true, adminEdit: true, external: false,
      user: { name: 'QA' }, projects: [{ id: 'pf1', name: 'Проект портфеля А', portfolio: true, lat: 41.3140, lng: 69.2830 }], activeProjectId: 'pf1', probeProjectId: '' } }, location.origin);
    await new Promise(r => setTimeout(r, 800));
    return { keys: Object.keys(PROJECTS), lProj: !!document.getElementById('lProj'),
             txt: document.getElementById('mlgd').innerText.replace(/\s+/g, ' '),
             tips: gProj.getLayers().map(l => (l.getTooltip && l.getTooltip()) ? String(l.getTooltip().getContent()) : '') };
  });
  ck('проекты из контекста в студию не попадают: только точка анализа', pf.keys.length === 1 && pf.keys[0] === 'project', pf.keys.join(','));
  ck('галочки «Проекты портфеля» нет', !pf.lProj);
  ck('в легенде нет блока «Проекты портфеля»', !/Проекты портфеля/.test(pf.txt), pf.txt.slice(0, 160));
  ck('на карте нет точек проектов', !pf.tips.some(t => /Проект портфеля/.test(t)), pf.tips.join(' | ').slice(0, 120));
