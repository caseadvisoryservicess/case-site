...
(async () => {
  ...
  ck('модули 4.75.0 зарегистрированы (инструменты, компоновка, агент, дерево, geo-direct)', v.tools === '4.75.0' && v.layout === '4.75.0' && v.agent === '4.75.0' && v.tree === '4.75.0' && v.direct === '4.75.0', JSON.stringify([v.tools, v.layout, v.agent, v.tree, v.direct]));
  ...
  const tb = await pg.evaluate(() => {
    ...
    const btns = [...tb.querySelectorAll('button')].map(b => b.getAttribute('data-a')), keys = [...tb.querySelectorAll('button kbd')].map(k => k.textContent);
    const cs = getComputedStyle(tb);
    return { btns, keys, centered: Math.abs((r.left + r.width / 2) - (mr.left + mr.width / 2)) < 40, bottom: mr.bottom - r.bottom, radius: cs.borderRadius, undoOff: tb.querySelector('[data-a=undo]').disabled, redoOff: tb.querySelector('[data-a=redo]').disabled, tips: [...tb.querySelectorAll('button')].every(b => b.getAttribute('data-tip')) };
  });
  ck('панель внизу по центру карты: отмена, повтор | пин, полигон, от руки, круг, маршрут, комментарий | стиль', tb.btns.join() === 'undo,redo,pin,poly,free,circle,route,note,style' && tb.centered && tb.bottom > 5 && tb.bottom < 60 && tb.tips, JSON.stringify(tb));
  ...
  await pg.waitForTimeout(500);
  const rings = await pg.evaluate(() => ({ radii: RADII.map(r => r.km), labels: [...document.querySelectorAll('#rings label')].map(l => l.textContent.trim()), saved: localStorage.getItem('caseos_rings_v1'), circles: (() => { const out = []; gRing.eachLayer(l => { if (l instanceof L.Circle) out.push(l.getRadius()); }); return out.sort((a, b) => a - b); })() }));
  ck('радиусы «0.7, 2, 4.5, 7» применены: четыре кольца нарисованы вокруг точки, выбор запомнен', rings.radii.join() === '0.7,2,4.5,7' && rings.labels.length === 4 && /0\.7 км/.test(rings.labels[0]) && rings.circles.join() === '700,2000,4500,7000' && rings.saved === '[0.7,2,4.5,7]', JSON.stringify(rings));
