...
echo "ok";`);
  ...
  r = await guest.call('auth.php', { action: 'register', name: 'Клиент Пробный', email: 'client@firm.test', password: 'client12345', company: 'Фирма', phone: '+998 90 000 00 00' });
  ...
  const again = await guest.call('auth.php', { action: 'register', name: 'Клиент Пробный', email: 'client@firm.test', password: 'other12345' });
  ...
  r = await guest.call('auth.php', { action: 'register', name: 'К', email: 'bad', password: '123' });
