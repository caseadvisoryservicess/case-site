...
(async () => {
  ...
  ck('Юнусабадский: 64 махалли, сумма оценок = 396 800 (Toshstat 01.07.2026), реестр Etirof 64 и 40 официальных границ, все 64 сопоставлены, 40 с границами (asking), остальные по сетке, порядок по населению', m1.n === 64 && near(m1.sum, 396800, 5) && m1.total === 396800 && /в реестре Etirof 64 махаллей, официальных границ 40/.test(m1.txt) && /в базе 64 \(сопоставлено с реестром 64\)/.test(m1.txt) && /396 800/.test(m1.txt.replace(/\u00a0/g, ' ')) && m1.desc && m1.allEst && m1.bounded === 40 && m1.official === 64 && m1.matched === 64 && m1.calibrated, JSON.stringify({ n: m1.n, sum: Math.round(m1.sum), txt: m1.txt.slice(0, 160), desc: m1.desc, allEst: m1.allEst, bounded: m1.bounded, matched: m1.matched }));
  ...
  ck('клик по махалле: выделена, подпись на карте, слой района из 64 кружков, 40 официальных и 24 расчётных границ и подписи, поле ручного населения открыто', m3.sel === 1 && m3.lbl && m3.edit && m3.name.replace(':', '') === m3.cur && m3.layers === 129, JSON.stringify(m3));
  ...
  /* v4.78.0 (замечание владельца): свой радиус вводится числом, не только три кнопки */
  await pg.fill('#amenR', '650'); await pg.press('#amenR', 'Enter'); await pg.waitForTimeout(250);
  const a3 = await pg.evaluate(() => ({ r: localStorage.getItem('caseos_amen_r'), on: document.querySelectorAll('#amen .amen-r button.on').length, val: document.getElementById('amenR').value, note: document.querySelector('#amen .amen-note').textContent, api: window.CASE_GEO_PROFILES.radius() }));
  ck('свой радиус 650 м: принят, запомнен, ни одна быстрая кнопка не подсвечена, пересчёт по 650 м', a3.r === '650' && a3.on === 0 && a3.val === '650' && /650 м/.test(a3.note) && a3.api === 650, JSON.stringify(a3));
  await pg.fill('#amenR', '9'); await pg.press('#amenR', 'Enter'); await pg.waitForTimeout(250);
  ck('радиус меньше 50 м поднимается до 50', await pg.evaluate(() => window.CASE_GEO_PROFILES.radius() === 50 && document.getElementById('amenR').value === '50'));
  await pg.click('#amen .amen-r button[data-r="500"]'); await pg.waitForTimeout(200);
  ...
  const p3 = await vis();
  ck('«Лизинг и продажи»: без анализа локации, махаллей и стороны дороги', !p3.analysis && !p3.mah && !p3.road && p3.rings && p3.layers, JSON.stringify(p3));
