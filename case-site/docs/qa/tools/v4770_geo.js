...
(async () => {
  ...
  ck('модули 4.77.0, восемь размеченных разделов, профиль на вкладках, «Что рядом» под точкой, демография загружена, 545 махаллей, 12 районов', v.mah === '4.77.0' && v.rs === '4.77.0' && v.pr === '4.77.0' && v.sects.join(',') === 'point,analysis,road,rings,bc,mah,layers,catch' && v.schema === 'case-demography/v1' && v.pts >= 12 && v.tabsSel && v.amenIn && v.poi === 545 && v.distOpts === 13, JSON.stringify(v));

  console.log('--- 2. Махалли по районам');
  await pg.selectOption('#mahDist', 'Yunusabad'); await pg.waitForTimeout(400);
  const m1 = await pg.evaluate(() => { const rows = [...document.querySelectorAll('#mahList .mah-row')]; const e = window.CASE_GEO_DEMO.estimateDistrict('Yunusabad'); const sum = e.rows.reduce((s, r) => s + (r.pop || 0), 0); return { n: rows.length, sum, total: e.total, txt: document.getElementById('mahSum').textContent, first: rows[0] ? rows[0].textContent : '', desc: rows.length > 1 ? (+rows[0].querySelector('.p').textContent.replace(/[^\d]/g, '')) >= (+rows[1].querySelector('.p').textContent.replace(/[^\d]/g, '')) : false, allEst: e.rows.every(r => r.pop != null && r.conf === 'modelled'), calibrated: e.calibrated }; });
  ck('Юнусабадский: 64 махалли, сумма оценок = 393 000 (официально), «из ~64 заявленных», порядок по населению, все оценки по сетке', m1.n === 64 && near(m1.sum, 393000, 5) && m1.total === 393000 && /64 махаллей в базе из ~64 заявленных/.test(m1.txt) && /393 000/.test(m1.txt.replace(/\u00a0/g, ' ')) && m1.desc && m1.allEst && m1.calibrated, JSON.stringify({ n: m1.n, sum: Math.round(m1.sum), txt: m1.txt.slice(0, 120), desc: m1.desc, allEst: m1.allEst }));
  ...
  ck('клик по махалле: выделена, подпись на карте, слой района из 64 кружков и подписи, поле ручного населения открыто', m3.sel === 1 && m3.lbl && m3.edit && m3.name.replace(':', '') === m3.cur && m3.layers === 65, JSON.stringify(m3));
  ...
  ck('ручное население 12 000: приоритет, помечено зелёным, сумма по району прежняя, запомнено', m4.pop === 12000 && m4.manual && m4.conf === 'verified' && near(m4.sum, 393000, 5) && m4.manualN === 1 && m4.saved === 12000 && m4.green && /вручную: 1/.test(m4.txt), JSON.stringify(m4));
  ...
  const s2 = await pg.evaluate(() => { const c = document.getElementById('card'); const t = c.textContent.replace(/\u00a0/g, ' '); return { open: c.classList.contains('open') && c.classList.contains('dm-open'), scopes: [...document.querySelectorAll('#dmScope option')].map(o => o.value), pop: /3 212 200/.test(t), hh: /747 023/.test(t), fem: /50,9%/.test(t), pyr: document.querySelectorAll('.dm-pyr rect').length, mod: document.querySelectorAll('.dm-pyr .dm-mod').length, bars: document.querySelectorAll('.dm-bar').length, rdeBars: document.querySelectorAll('.dm-bar.rde').length, trRows: document.querySelectorAll('.dm-tr tbody tr').length, fc2036: /2036 · прогноз/.test(t), method: /CAGR/.test(t), lines: document.querySelectorAll('.dm-line').length, need: /Требуемые входные данные/.test(t) && /Пятилетние возрастные группы/.test(t), src: /Источники/.test(t) && /stat\.uz/.test(t), as: /Допущения/.test(t) && /национальной пропорции/.test(t), kipc2025: /КИПЦ, 2025/.test(t) }; });
  ck('карточка по городу: охваты город + 12 районов + выбранная махалля, население 3 212 200, домохозяйств 747 023, женщин 50,9%, пирамида 32 столбца в полутоне, 13 разделов КИПЦ (4 RDE), три ряда тренда, прогноз 2036 с методом CAGR, две диаграммы, допущения, входные данные, источники', s2.open && s2.scopes.length === 14 && s2.scopes[0] === 'city' && s2.scopes.filter(s => /^d:/.test(s)).length === 12 && s2.scopes.filter(s => /^m:/.test(s)).length === 1 && s2.pop && s2.hh && s2.fem && s2.pyr === 32 && s2.mod === 32 && s2.bars === 13 && s2.rdeBars === 4 && s2.trRows === 3 && s2.fc2036 && s2.method && s2.lines === 2 && s2.need && s2.src && s2.as && s2.kipc2025, JSON.stringify(s2));
  await pg.selectOption('#dmScope', 'd:Yunusabad'); await pg.waitForTimeout(300);
  const s3 = await pg.evaluate(() => { const t = document.getElementById('dmBody').textContent.replace(/\u00a0/g, ' '); return { pop: /393 000/.test(t), scope: /Юнусабадский район/.test(t), hh: /91 395/.test(t) }; });
  ck('охват «Юнусабадский район»: 393 000 жителей, 91 395 домохозяйств', s3.pop && s3.scope && s3.hh, JSON.stringify(s3));
  ...
  ck('CSV выгружен: охват, население, группы, КИПЦ, прогноз, допущения', !!dlf && /Юнусабадский/.test(csvTxt) && /"население";"393000"/.test(csvTxt) && /возраст 0-4 мужчины/.test(csvTxt) && /КИПЦ 01/.test(csvTxt) && /прогноз/.test(csvTxt) && /допущение/.test(csvTxt), dlf ? csvTxt.slice(0, 80) : 'нет скачивания');
