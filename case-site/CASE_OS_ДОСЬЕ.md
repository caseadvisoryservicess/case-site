...
и `claude/new-session-2kq1ff` (одинаковое содержимое). Последний коммит: `fbd79d3` «CASE OS v4.73.0: свой
...
Разделы 1-12 составлены по файлам репозитория на коммит v4.73.0. Раздел 13 - выжимка четырёх
последних выпусков (v4.70.3 ... v4.73.0), включая решения владельца и найденные ловушки.
...
| `CHANGELOG_CASE_OS_v4.51.0.md` ... `CHANGELOG_CASE_OS_v4.73.0.md` | Журналы изменений по релизам (v4.68.0 описан внутри файла v4.69.0; v4.53.1 и v4.57.1 внутри v4.53 и v4.57.0). |
...
| `os/DEPLOY.md` | Краткая инструкция установки текущего релиза (v4.73.0). |
| `os/SHA256SUMS_v4.73.0.txt` | Манифест контрольных сумм релиза (166 строк). Ровно один файл манифеста в `os/` - это проверяет `release_invariants.js`. |
...
| `docs/standalone/` | `CASE_OS_Geo_Analytics.html` (1.1 МБ, автономная гео-версия, открывается с диска) + README. |
...
### 3.2. Модули и версии (v4.73.0, `os/index.html`)
...
`window.parent`): `v420-geo-studio` 4.62.0, `v4530-geo-export` 4.58.0, `v4600-sun-wind` 4.61.0,
`v4630-huff` 4.63.0, `v4730-geo-agent` 4.73.0 (заменил `v4720-geo-assistant` из v4.72.0).

Порядок подключения и назначение (по заголовочным комментариям файлов):

| Файл (`?v=` в v4.73.0) | Назначение |
|---|---|
...
| `v420-geoanalytics.js?v=4.73.0` | Мост между ядром и гео-студией: iframe, контекст проекта, сохранение GEO_DATA через `geo_state.php`, черновик при отказе, защита от обвала базы (мост ассистента v4.72.0 удалён). |
...
- `os/sw.js`: `CACHE='case-os-v4730'` для v4.73.0 (число = версия без точек; проверяется
...
Объекты на карте (`objT`). Левая панель карты: **Проект** (список проектов, «▣ Аналитика по
проекту», «⤓ Выгрузить PDF · Excel · PPTX», «⚙ Настройки», «☀ Солнце, ветер и кибла», «◱ Доля
рынка (модель Хаффа)»), **Гео-агент** (v4.73.0, свой движок), **Анализ локации** (клик = отчёт по точке,
...
Группы в «Слои и стиль»: Проекты CASE (проекты портфеля), Бизнес-центры, HoReCa, Медицина и
...
3. Владельцу: раздел 8 (хостинг), раздел 10 (что открыто) и 13.5 (что дальше). Архив для хостинга
