...
### v4.74.0 ... v4.77.0
Платформа только для геоаналитики, панель инструментов и агент справа, бизнес-центры внутри
студии и модель доступа, махалли и демография, сторона дороги, профили, оферта, кабинет, бот.
Подробно - разделы 13.9-13.12.

### v4.78.0
Пробелы дорожной карты закрыты (границы махаллей вручную, демография во всей аналитике и
выгрузках, тренды расходов, «Мои данные», бесплатный доступ «Ищу офис»); данные Ташкента от
владельца: реестр 585 махаллей Etirof с кодами, 402 границы из кадастра open.ngis.uz, население
и показатели районов, генплан и налоговые зоны в точке; модуль НГИС из браузера.
Проверки `v4780_studio.js`, `v4780_platform.js`. Подробно - раздел 13.13.
...
## 13. Текущее состояние: v4.70.3 ... v4.78.0 (сентябрь 2026)
...
### 13.10. v4.75.0 - панель инструментов, контекстное меню, агент справа, зоны охвата

Десять замечаний владельца к студии со скриншотами Felt и Google Earth. Сделано (см.
`CHANGELOG_CASE_OS_v4.75.0.md`): клик по карте ничего не открывает (флажок «клик = отчёт по точке»
убран); `os/v4750-geo-tools.js` рисует внизу по центру карты панель «отмена, повтор | пин P |
полигон O | от руки F | круг C | маршрут R | комментарий M | стиль карты» со стеком отмены,
хоткеями и контекстным меню по правому клику (метка, круг, маршрут, переместиться, сведения,
копировать и вставить координаты); пин, полигон и круг уходят гео-агенту как точка и фигуры зоны;
маршрут по OSRM из браузера с запасным расстоянием по прямой; комментарии в localStorage.
`os/v4750-geo-layout.js` переносит агента в правый выдвижной ящик с одной кнопкой (фокус при
автоматическом открытии остаётся на карте), дописывает обозначения в плашку «На экране», убирает
флаг из атрибуции и даёт «Зоне охвата» произвольные радиусы. Агент 4.75.0 получил `draw_circle` и
`catchment` по методике CASE: PTA / STA / TTA по изохронам OSRM (или радиусам 0,4 км/мин, об этом
сказано), домохозяйства 4,5, RDE $349/чел/год, при GLA и ставке требуемый оборот при rent-to-sales
15%, доли 75/20/5, capture rate с ориентиром 0,39% и предупреждением выше 2%, список «Требуемые
входные данные». Из дерева убран узел «Подложка карты», из левой панели секция «Легенда» и кнопки
«Зона: рисовать». Проверки: `v4750_geo_tools.js` (65), обновлены `v4730_geo_agent.js`,
`v4740_geo_tree.js`, `v4740_geo_only.js`, `v4732_standalone_build.js`. Ловушки: автооткрытие ящика
не должно забирать фокус (хоткеи печатались в поле); «по центру» считать от контейнера карты;
Дугласа-Пекера с совпадающими концами; маркер Leaflet нельзя класть в JSON.

### 13.11. v4.76.0 - студия по замечаниям, бизнес-центры внутри, модель доступа

Замечания владельца после v4.75.0: пин без колец, свой цвет у каждого кольца, полный экран
карты, левая панель сворачивается, подложка в панели инструментов, убрать «Сравнение стран»,
«Точку на карте» и строку-подсказку, слить экран «Geo Platform: бизнес-центры» со студией (те же
данные путали), и модель доступа: администратор, сотрудники, клиенты, настройки на каждого,
журнал по пользователю, корзина удалённого, подписка со сроком и предупреждением за 5 дней,
регистрация, демо с ограниченными данными. Сделано (см. `CHANGELOG_CASE_OS_v4.76.0.md`):
`v4760-geo-bc.js` (фильтры, список, сравнение, показатели, тепловая карта ставок; студия
спрашивает `CASE_GEO_BC.pass` в `renderBC` и `catchSummary`), `v4760-geo-caps.js` (права внутри
студии: демо 40 БЦ, без выгрузки и правок), `v4760-access.js` (экран входа с регистрацией и демо,
баннер срока, карточки «Заявки», «Доступ, подписка и журнал», «Корзина геоданных»), серверная
часть в `lib.php`, `auth.php` (`register`, `demo`), `users.php` (`set_profile`, `approve`, `log`),
`geo_state.php` (таблица `geo_trash`, `restore_trash`), `state.php` (демо не пишет), роли `CL` и
`DEMO`, миграция `2026_09_17_v4760_access.sql` (код дочинивает колонки сам). Файл
`os/geo-platform.html` удалён; автономный MVP остаётся продуктом для клиентов. Проверки:
`v4760_access_api.js` (настоящий PHP на встроенном сервере с SQLite, 35), `v4760_access.js`
(18), остальные обновлены. Ловушки: ядро пересобирает `S.user` без новых полей; `BC` в студии
`const`; `session_destroy()` теряет CSRF; клиентские пресеты ролей должны знать CL и DEMO.

### 13.12. v4.77.0 - махалли и демография, сторона дороги, профили, оферта, кабинет, бот

Дорожная карта владельца после v4.76.0 «всё сразу»: махалли как подкатегории районов с
населением и структурой; вся аналитика со структурой населения (пол, пятилетние группы
диаграммой, доход, домохозяйства, расходы и объём рынка), у каждого показателя тренд и прогноз
на десять лет; сторона дороги по типу проекта; у каждого типа пользователя своя панель
(ищущим офис удобства: парковка, метро, остановки; девелоперам, управляющим, консалтингу,
лизингу своё); публичная оферта с согласием галочкой при регистрации и внутри системы;
личный кабинет; бот для обращений. Сделано (см. `CHANGELOG_CASE_OS_v4.77.0.md`):
`v4770-geo-mahalla.js` (545 махаллей по 12 районам; население махалли = население района ×
доля по калиброванной сетке в 900 м вокруг точки, ручное значение и официальная граница из
`data/mahalla_boundaries.geojson` имеют приоритет; карточка «Демография и рынок» по охвату
город / район / махалля / зона охвата: пол, три группы, пирамида 16 групп на SVG, доход, КИПЦ
13 разделов с объёмом рынка, RDE, capture rate 0,394%, тренд и прогноз CAGR на 10 лет, допущения,
требуемые входные данные, источники, CSV), `data/demography_tashkent.json` (каждое число с
источником и уверенностью; null не выдумывается; реестр источников и ссылки на слои Open Data
Tashkent из исследования `docs/data/tashkent_city_gis_research_2026-09-18.md`),
`v4770-geo-roadside.js` (ближайший проезжий отрезок из `load_roads` агента, направление «в
центр» к [41.3111, 69.2797], сторона по правостороннему движению, односторонние и поперечные
дороги, 11 типов проектов с предпочтением стороны, стрелки и подпись на карте, факт в журнале,
фразы агенту), `v4770-geo-profiles.js` (шесть профилей, разделы размечены `data-sect` по
тексту h3, профиль из прав учётной записи или localStorage, демо стартует с «Ищу офис»; блок
«Что рядом» по `CASE_GEO_POI.breakdown`, `METRO`, `PHARM`, `MEDPTS`, `BC`), `offer.html` (оферта
1.0), `v4770-cabinet.js` (окно согласия, меню, кабинет, обратная связь, карточка «Обращения»),
`api/feedback.php` (send, status, список, `action:'bot'` по токену без сеанса),
`auth.php` (`accept_offer`, `update_profile`, `change_password`, `my_log`, регистрация требует
`offer_accepted`), `bot/telegram_feedback_bot.js` (Node, long polling, вне хостинга: PHP там без
интернета). Проверки: `v4770_geo.js` (24), `v4770_cabinet.js` (28), `v4770_api.js` (46, настоящий
PHP). Ловушки: модули студии, чей `install` завершается синхронно, стартуют раньше
`v4760-geo-caps.js`, поэтому права надо ждать; `client_ip()` и throttle живут в `auth.php`, а не в
`lib.php`; `apiPOST` ядра повторяет запрос после 403; станции метро в студии `[lat, lng, name]`;
`php -r` через оболочку теряет `$`.

### 13.13. v4.78.0 - пробелы дорожной карты закрыты, данные Ташкента владельца, НГИС

Замечание владельца «ты не пропустил мои задачи?» после v4.77.0: закрыты границы махаллей
вручную с карты (последний полигон инструментов O/F становится границей выбранной махалли,
`geoSetMahallaBoundary` пишет поле `boundary` в запись мастер-базы, без права правок копия в
`caseos_mahalla_bounds_v1`; население внутри границы дробными ячейками `popInZone`), демография
во всей аналитике (раздел L отчёта по точке и PDF, I/I2 на вкладке «Аналитика», лист Excel и
слайд PPTX через `exportRows`), тренд и прогноз расходов и RDE, «Мои данные» в кабинете,
бесплатный ограниченный доступ «Ищу офис» при регистрации (`tier` free/full, `limited`,
`free_office_access`). Данные владельца за день (см. `CHANGELOG_CASE_OS_v4.78.0.md`): разбор
геопортала open.ngis.uz (`docs/data/ngis/`, ArcGIS REST db.ngis.uz, анонимный доступ, CORS открыт,
запросы только из браузера пользователя), пакеты `docs/data/tashkent_gis_dataset` (v1: слой
хокимията, 585 махаллей на трёх языках, ряды Toshstat по районам, 38 показателей города) и
`docs/data/tashkent_gis_master` (v2: реестр Etirof с кодами SOATO махалли, 402 полигона кадастра
с геодезической площадью, SOATO районов уровня A, население 01.10.2025 и 01.01.2026, 25
источников, 12 позиций недостающих данных, правило дублей имён), сырые выгрузки НГИС (генплан
7 419 зон, налоговые зоны; кадастровые участки по замечанию владельца не берутся, они меняются
слишком быстро; сырые файлы в репозиторий не кладутся).
Сборка данных: `build_tashkent_data.js <v1> <v2>` (сопоставление двух реестров внутри района:
551 точно, 6 по словарю, 4 по инициалам, 21 по близости написания, 3 по остатку как
переименования; пишет `data/mahalla_registry_tashkent.json` схемы v2 и дополняет
`data/demography_tashkent.json`, печатает `DIST`), `ngis_to_studio.js` (мастер-набор или сырой
слой → `data/mahalla_boundaries.geojson`, код = кадастровый номер). Студия: `v4770-geo-mahalla.js`
сопоставляет 545 точек базы с реестром (имя в английской транслитерации против узбекской
латиницы и кириллицы, полигон, близость написания; одна запись одной строке), добавляет
официальные махалли с полигоном без точки в базе строками «реестр», присваивает полигон одной
строке (вручную > код > ближайшая к центру), считает площадь, плотность; для района
ряд населения Toshstat, блоки фактов района и реестра махалли; `mahallaAt(lat, lng)`.
`v4780-geo-ngis.js`: границы живым запросом, генплан и налоговая зона в точке сначала из
локальных выгрузок (PIP по bbox), живой запрос как запасной, слой генплана на canvas с легендой.
Проверки: `v4780_studio.js` (24), `v4780_platform.js`, `v4770_api.js` раздел 7, `v4770_geo.js`
под новые числа. Ловушки: студия при загрузке подменяет `renderProj` и `analytics`, обёртки
ставятся заново полторы минуты, раздел на вкладке дорисовывается по клику; точка проекта по
умолчанию есть при старте, поэтому `loadLocal` генплана уходит сразу (в тесте маршрут-заглушка
ставится до `open()`); стаб НГИС должен нести настоящие коды махалли, иначе полигон уйдёт другой
строке по коду; `git checkout` файла демографии перед пересборкой (сборка идемпотентна от
состояния v4.77); площади `DIST` по границам 2024 сходятся с геодезическими кадастра.
