...
(function(){
  ...
  var T = {
    ru:{
      ...
      myBonus:'Мой бонус', teamBonus:'Комиссии команды', pipeline:'Воронка', weighted:'Взвешенный прогноз', lost:'Упущено', earned:'Накоплено', coming:'Придёт при закрытии',
  ...
  function installCSS(){
    ...
    var css = ''+
      ...
      '.v32-kpis{display:grid;grid-template-columns:repeat(auto-fit,minmax(138px,1fr));gap:9px}.v32-kpi{border:1px solid var(--border);border-radius:12px;padding:10px;background:var(--soft)}.v32-kpi small{display:block;color:var(--muted);font-size:11px}.v32-kpi b{display:block;color:var(--ink);font-size:20px;margin-top:3px}.v32-kpi.warn b{color:#b56b00}.v32-kpi.bad b{color:var(--red-d)}.v32-kpi.v32-click{cursor:pointer;transition:border-color .12s,box-shadow .12s}.v32-kpi.v32-click:hover{border-color:var(--red);box-shadow:0 2px 10px rgba(158,0,0,.08)}'+
      ...
      '@media(max-width:1100px){.v32-grid,.v32-grid.three{grid-template-columns:repeat(2,1fr)}.v32-kpis{grid-template-columns:repeat(2,1fr)}.v32-split{grid-template-columns:1fr}}@media(max-width:640px){.v32-grid,.v32-grid.three,.v32-row2,.v32-row3{grid-template-columns:1fr}.v32-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.v32-table{font-size:11px}.v32-head{display:block}.v32-tools{display:block}.v32-tools>*{margin:4px 0;width:100%}}';
  ...
  function hubHTML(){
    ...
    /* v4.34: один ряд КЛИКАБЕЛЬНЫХ плиток вместо «плитки + дублирующие карточки с теми же числами»;
       без служебных текстов (routeHint) и устаревшей метки «CASE OS v4.9.3» */
    var tiles=[
      [tr('tasksToday'), is.overdue.length, "go('v32_action')", is.overdue.length?'bad':''],
      [tr('requests'), window.BRAND_REQUESTS.length, "go('v32_requests')", ''],
      [tr('investorLeads'), window.INVESTOR_REQUESTS.length, "go('v32_investors')", ''],
      [tr('sales'), window.SALES_ASSETS.length, "go('v32_sales')", ''],
      [tr('partners'), window.CASE_PARTNERS.length, "go('v32_partners')", ''],
      [tr('coming'), money(sum(rows,'pipeline')), '', ''],
      [tr('weighted'), money(sum(rows,'weighted')), '', '']
    ];
    return '<div class="v32-card v32-hub"><div class="v32-head"><div><h2>'+h(tr('action'))+'</h2></div><div><span class="v32-pill">'+h(tr('qaAbbr'))+'</span><span class="v32-pill">'+h(tr('offerAbbr'))+'</span><span class="v32-pill">'+h(tr('ddAbbr'))+'</span></div></div>'+
      '<div class="v32-kpis">'+tiles.map(function(t){ var click=t[2]?' v32-click" role="button" tabindex="0" title="Открыть раздел" onclick="'+t[2]+'"':'"'; return '<div class="v32-kpi '+t[3]+click+'><small>'+h(t[0])+'</small><b>'+t[1]+'</b></div>'; }).join('')+'</div></div>';
  }
  ...
  function actionRowsHTML(){ var is=issues(), rows=allBonusRows(); var tasks=[]; is.overdue.slice(0,8).forEach(function(u){ tasks.push([tr('tasksToday'),u.code,(u.dates&&u.dates[0]&&u.dates[0][0])||tr('nextAction'),u.broker||'-']); }); window.BRAND_REQUESTS.filter(function(r){return r.due&&r.due<=addDays(7);}).slice(0,8).forEach(function(r){tasks.push([tr('requests'),r.brand,r.nextAction||'-',r.broker||'-']);}); window.INVESTOR_REQUESTS.filter(function(r){return r.due&&r.due<=addDays(7);}).slice(0,8).forEach(function(r){tasks.push([tr('investorLeads'),r.investor,r.nextAction||'-',r.broker||'-']);}); window.SALES_ASSETS.filter(function(a){return a.due&&a.due<=addDays(7);}).slice(0,8).forEach(function(a){tasks.push([tr('sales'),a.name,a.nextAction||'-',a.broker||'-']);});
    return '<div class="v32-split"><div class="v32-card"><div class="v32-head"><div><div class="v32-eyebrow">ACTION</div><h3>'+h(tr('tasksToday'))+'</h3></div></div>'+(tasks.length?'<div class="tbl-scroll"><table class="v32-table"><thead><tr><th>'+h(tr('source'))+'</th><th>'+h(tr('property'))+' / '+h(tr('brand'))+'</th><th>'+h(tr('nextAction'))+'</th><th>'+h(tr('broker'))+'</th></tr></thead>'+tasks.map(function(t){return '<tr><td>'+h(t[0])+'</td><td><b>'+h(t[1])+'</b></td><td>'+h(t[2])+'</td><td>'+h(t[3])+'</td></tr>';}).join('')+'</table></div>':'<div class="v32-empty">'+h(tr('noRows'))+'</div>')+'</div><div class="v32-card"><div class="v32-head"><div><div class="v32-eyebrow">BONUS</div><h3>'+(canManage()?h(tr('teamBonus')):h(tr('myBonus')))+'</h3></div></div><div class="v32-kpis" style="grid-template-columns:1fr 1fr"><div class="v32-kpi"><small>'+h(tr('earned'))+'</small><b>'+money(sum(rows,'earned'))+'</b></div><div class="v32-kpi"><small>'+h(tr('coming'))+'</small><b>'+money(sum(rows,'pipeline'))+'</b></div><div class="v32-kpi"><small>'+h(tr('weighted'))+'</small><b>'+money(sum(rows,'weighted'))+'</b></div><div class="v32-kpi bad"><small>'+h(tr('lost'))+'</small><b>'+money(sum(rows,'lost'))+'</b></div></div></div></div>'; }
  ...
  function scrSaleEnabled(oid){try{return (typeof objSaleOn==='function')?objSaleOn(oid):true;}catch(e){return true;}}
  function scrSaleOffCard(oid){return '<div class="v32-card"><div class="v32-empty">'+h(scrT('saleOff'))+(scrCanEdit()&&typeof addObjectForm==='function'?'<div style="margin-top:10px"><button class="btn sm" onclick="addObjectForm(\''+oid+'\')">'+h(scrT('objSettings'))+'</button></div>':'')+'</div></div>';}
  function scrRegistry(){
    ...
    if(!scrSaleEnabled(oid))return scrSaleOffCard(oid);
  ...
  /* Отказной лист SCR — общий с LCR по объекту (REFUSALS[objId]). Тот же движок buildTable. */
  function scrRefus(){
    var oid=(typeof S==='object'&&S.obj)?S.obj:'ALL';
    var allObj=(oid==='ALL');
    ...
    cols.push({k:'reason',label:'Причина отказа',get:function(r){return r.reason||'';},sortable:true,filter:'text'});
    cols.push({k:'date',label:'Дата',get:function(r){return r.date||'';},sortable:true});
    cols.push({k:'by',label:'Кто внёс',get:function(r){return r.by||'';},sortable:true,filter:'select',render:function(r){return r.by?h(r.by):'<span class="mut">-</span>';}});
    if(canEdit)cols.push({k:'_del',label:'',get:function(){return '';},render:function(r){return '<button class="btn ghost sm" title="Удалить" onclick="deleteRefus(\''+r.obj+'\','+r._i+')">🗑</button>';}});
    var tbl=(typeof buildTable==='function')?buildTable('scr_refus',cols,list):'';
    var addBtn=canEdit&&typeof addRefus==='function'?'<div style="margin-top:10px"><button class="btn sm" onclick="addRefus()">+ Отказ</button></div>':'';
    return '<div class="v32-card"><div style="display:flex;gap:8px;align-items:center;margin-bottom:6px"><b style="font-size:14px">'+h(scrT('refus'))+' (SCR)</b><span class="v32-pill" style="font-size:11px">'+list.length+'</span></div><p class="sub" style="margin:0 0 10px">Журнал отказов по продаже: кто отказался, что и на каких условиях предлагали, причина. Общий с арендой (LCR) по объекту — не предлагать повторно + рыночные сигналы.</p>'+tbl+addBtn+'</div>';
  }
  function renderSaleAssets(){
    ...
    return tools+
      ...
      '<div class="v32-card"><div class="v32-head"><div><div class="v32-eyebrow">SALE ASSETS</div><h3>'+h(tr('property'))+'</h3></div></div>'+(assets.length?'<div class="tbl-scroll"><table class="v32-table"><thead><tr><th>'+h(tr('property'))+'</th><th>'+h(tr('city'))+'</th><th>'+h(tr('areaMin'))+'</th><th>'+h(tr('price'))+'</th><th>'+h(tr('stage'))+'</th><th>'+h(tr('nextAction'))+'</th><th>'+h(tr('broker'))+'</th><th>'+h(tr('actions'))+'</th></tr></thead>'+assets.map(function(a){return '<tr><td><b>'+h(a.name)+'</b><div class="mut">'+h(a.type||'')+' · '+h(a.visibility||'')+'</div></td><td>'+h(a.country||'')+'<br><span class="mut">'+h(a.city||'')+'</span></td><td>'+h(a.area||'-')+'</td><td>'+money(a.price||0)+'<div class="mut">'+h(tr('commission'))+': '+money(a.expectedCommission||((+a.price||0)*(+a.commissionRate||0)/100))+'</div></td><td>'+h(stageLabel(a.stage))+'</td><td>'+h(a.nextAction||'-')+'<div class="mut">'+h(a.due||'')+'</div></td><td>'+h(a.broker||'-')+'</td><td><button class="btn ghost sm" onclick="v32MatchBuyers(\''+a.id+'\')">'+h(tr('match'))+'</button> <button class="btn ghost sm" onclick="v32AssetForm(\''+a.id+'\')">'+h(tr('edit'))+'</button></td></tr>';}).join('')+'</table></div>':'<div class="v32-empty">'+h(tr('noRows'))+'</div>')+'</div>'+
      '<div class="v32-card"><div class="v32-head"><div><div class="v32-eyebrow">BUYERS</div><h3>'+h(tr('buyer'))+'</h3></div></div>'+(buyers.length?'<div class="tbl-scroll"><table class="v32-table"><thead><tr><th>'+h(tr('buyer'))+'</th><th>'+h(tr('country'))+' / '+h(tr('city'))+'</th><th>'+h(tr('budget'))+'</th><th>'+h(tr('areaMin'))+'-'+h(tr('areaMax'))+'</th><th>'+h(tr('broker'))+'</th><th>'+h(tr('actions'))+'</th></tr></thead>'+buyers.map(function(b){return '<tr><td><b>'+h(b.name)+'</b><div class="mut">'+h(b.assetType||'')+'</div></td><td>'+h(b.country||'')+'<br><span class="mut">'+h(b.city||'')+'</span></td><td>'+money(b.budgetMin||0)+' - '+money(b.budgetMax||0)+'</td><td>'+h(b.areaMin||'-')+' - '+h(b.areaMax||'-')+'</td><td>'+h(b.broker||'-')+'</td><td><button class="btn ghost sm" onclick="v32BuyerForm(\''+b.id+'\')">'+h(tr('edit'))+'</button></td></tr>';}).join('')+'</table></div>':'<div class="v32-empty">'+h(tr('noRows'))+'</div>')+'</div>';
  }
  ...
  window.v32MatchRequest=function(rid){ ensureData(); var r=window.BRAND_REQUESTS.find(function(x){return x.id===rid;}); if(!r)return; var units=[]; try{ units=(Array.isArray(U)?U:[]).filter(function(u){ var o=typeof objById==='function'?objById(u.obj):null; var cityOk=!r.city || !o || String(o.city||'').toLowerCase().indexOf(String(r.city).toLowerCase())>=0; var area=+u.area||0; var areaOk=(!r.areaMin||area>=r.areaMin)&&(!r.areaMax||area<=r.areaMax); var open=['vac','neg','off','res'].indexOf(u.status)>=0; return cityOk&&areaOk&&open; }).slice(0,20); }catch(e){}
    var html='<div class="v32-card"><div class="v32-head"><div><div class="v32-eyebrow">MATCH</div><h3>'+h(r.brand)+'</h3><p>'+h(r.city||'-')+' · '+h(r.areaMin||0)+'-'+h(r.areaMax||0)+' m² · '+h(r.preferences||'')+'</p></div></div>'+(units.length?'<div class="tbl-scroll"><table class="v32-table"><thead><tr><th>'+h(tr('property'))+'</th><th>'+h(tr('areaMin'))+'</th><th>'+h(tr('stage'))+'</th><th>'+h(tr('actions'))+'</th></tr></thead>'+units.map(function(u){ var o=typeof objById==='function'?objById(u.obj):{}; return '<tr><td><b>'+h((o&&o.name)||u.obj)+'</b><div class="mut">'+h(u.code)+' · '+h(u.floor||'')+'</div></td><td>'+h(u.area||'-')+'</td><td>'+h(u.status||'')+'</td><td><button class="btn ghost sm" onclick="v32RequestToUnit(\''+rid+'\',\''+u.id+'\')">'+h(tr('shortlist'))+'</button></td></tr>';}).join('')+'</table></div>':'<div class="v32-empty">'+h(tr('noRows'))+'</div>')+'</div>'; modal(tr('match'),html,'<button class="btn" onclick="v32CloseModal()">OK</button>'); };
  ...
  window.v32MatchBuyers=function(aid){ var a=window.SALES_ASSETS.find(function(x){return x.id===aid;}); if(!a)return; var buyers=window.SALES_BUYERS.filter(function(b){ var price=+a.price||0, area=+a.area||0; var budgetOk=(!b.budgetMin||price>=b.budgetMin)&&(!b.budgetMax||price<=b.budgetMax); var areaOk=(!b.areaMin||area>=b.areaMin)&&(!b.areaMax||area<=b.areaMax); var cityOk=!b.city||!a.city||String(b.city).toLowerCase()===String(a.city).toLowerCase(); return budgetOk&&areaOk&&(cityOk||!b.city); }); var html='<div class="v32-card"><h3>'+h(a.name)+'</h3>'+(buyers.length?'<div class="tbl-scroll"><table class="v32-table"><thead><tr><th>'+h(tr('buyer'))+'</th><th>'+h(tr('budget'))+'</th><th>Contact</th></tr></thead>'+buyers.map(function(b){return '<tr><td><b>'+h(b.name)+'</b><div class="mut">'+h(b.assetType||'')+'</div></td><td>'+money(b.budgetMin||0)+' - '+money(b.budgetMax||0)+'</td><td>'+h(b.contact||'-')+'</td></tr>';}).join('')+'</table></div>':'<div class="v32-empty">'+h(tr('noRows'))+'</div>')+'</div>'; modal(tr('match'),html,'<button class="btn" onclick="v32CloseModal()">OK</button>'); };
