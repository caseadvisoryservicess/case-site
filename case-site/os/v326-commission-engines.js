...
(function(){
  ...
  window.v326LeaseReport=function(){var deals=visibleLeaseDeals(), led=leaseLedgerRows(deals); var rows='<div class="tbl-scroll"><table class="v326-table"><thead><tr><th>'+h(t('participant'))+'</th><th>'+h(t('role'))+'</th><th>'+h(t('paid'))+'</th><th>'+h(t('pending'))+'</th><th>'+h(t('weighted'))+'</th><th>'+h(t('lost'))+'</th></tr></thead>'; var g={}; led.forEach(function(r){var k=r.name+'|'+r.role; g[k]=g[k]||{name:r.name,role:r.role,earned:0,pending:0,weighted:0,lost:0}; ['earned','pending','weighted','lost'].forEach(function(x){g[k][x]+=r[x];});}); Object.values(g).forEach(function(r){rows+='<tr><td>'+h(r.name)+'</td><td>'+h(r.role)+'</td><td>'+money(r.earned)+'</td><td>'+money(r.pending)+'</td><td>'+money(r.weighted)+'</td><td>'+money(r.lost)+'</td></tr>';}); rows+='</table></div>'; modal(t('reports'),'<div class="v326-form">'+chips('<span>'+h(t('accounting'))+'</span><span>'+h(t('google'))+'</span>')+rows+'</div>','<button class="btn ghost" onclick="v326ExportLease()">'+h(t('export'))+'</button><button class="btn" onclick="v326CloseModal()">OK</button>');};
