...
(function(){
  ...
  function injectDashCards(){var main=document.getElementById('main'); if(!main||main.querySelector('.v326-dash'))return; ensureData(); var deals=visibleLeaseDeals(), sup=visibleSupplierDeals(); var leaseSum=sum(deals.map(function(d){return {x:leaseCalc(d).C};}),'x'); var supSum=sum(sup.map(function(d){return {x:supplierCalc(d).caseCommission};}),'x'); var html='<div class="v326-card v326-dash"><div class="v326-head"><div><div class="v326-eye">COMMISSION ENGINES</div><h3>'+h(t('lease'))+' / '+h(t('supplier'))+'</h3><p class="mut">'+h(t('privacy'))+'</p></div><div><button class="btn ghost sm" onclick="go(\'v326_lease\')">'+h(t('lease'))+'</button> <button class="btn ghost sm" onclick="go(\'v326_suppliers\')">'+h(t('supplier'))+'</button></div></div><div class="v326-kpis"><div class="v326-kpi"><small>'+h(t('lease'))+'</small><b>'+money(leaseSum)+'</b></div><div class="v326-kpi"><small>'+h(t('supplier'))+'</small><b>'+money(supSum)+'</b></div><div class="v326-kpi warn"><small>'+h(t('pending'))+'</small><b>'+money(sum(leaseLedgerRows(deals),'pending'))+'</b></div><div class="v326-kpi"><small>'+h(t('clientSavings'))+'</small><b>'+money(sum(sup.map(function(d){return {x:supplierCalc(d).clientSaving};}),'x'))+'</b></div><div class="v326-kpi"><small>'+h(t('openParams'))+'</small><b>ASH</b></div></div></div>'; var after=main.querySelector('.v32-hub')||main.querySelector('.v31-practice-card')||main.querySelector('.ph'); if(after)after.insertAdjacentHTML('afterend',html);}
