...
if ($m === 'POST') {
  ...
  // v4.35 (P1-6): по-ключевое слияние — клиент может прислать только изменённые домены,
  // отсутствующие в запросе ключи сохраняют серверное значение. Старые клиенты шлют весь
  // блоб — для них поведение не меняется (все ключи присутствуют и перекрывают старые).
  if (is_array($oldData) && $oldData) $incoming = array_replace($oldData, $incoming);
