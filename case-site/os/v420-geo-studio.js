...
(function(){
  ...
  function dataManager(){
    ...
    document.getElementById('dataT').innerHTML=(GEO_CAN_EDIT?'':'<div class="geo-notice">Режим просмотра: редактирование отключено для этой роли.</div>')
      +'<div class="geo-master-banner"><div><b>Мастер-база Ташкента · '+esc(bm.version||'4.8.1')+'</b><span>'+(bm.records||q.total).toLocaleString('ru-RU')+' объектов с координатами · '+(bm.business_centers||BC.length)+' БЦ · '+(bm.medicine||MEDPTS.length)+' медицина · '+(bm.pharmacies||PHARM.length)+' аптек · '+(bm.restaurants||GEO_POI.restaurants.length)+' ресторанов · '+(bm.cafes||GEO_POI.cafes.length)+' кафе · '+(bm.mahallas||0)+' махаллинских комитетов</span></div>'+(GEO_CAN_EDIT?'<div class="geo-master-links"><a href="api/geo_master.php?file=master.csv" download>Master CSV</a><a href="api/geo_master.php?file=master.geojson" download>GeoJSON</a><a href="api/geo_master.php?file=research_queue.csv" download>Очередь проверки</a><a href="api/geo_master.php?file=qa_issues.csv" download>QA-ошибки</a></div>':'')+'</div>'
