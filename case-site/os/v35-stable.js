(function(){
  ...
  function refreshBrandSubDatalist(){var cat=$('bd_cat'), sub=$('bd_sub'), dl=$('brandSubDL'); if(cat&&dl){var list=allSubs(cat.value); dl.innerHTML=list.map(function(x){return '<option value="'+h(x)+'">';}).join(''); if(sub && sub.value && list.length && list.indexOf(sub.value)<0){sub.title='Эта подкатегория не входит в выбранную категорию, но может быть сохранена вручную.';}}
    var td=$('brandTypeDL'); if(cat&&td){var tl=(window.CATTYPES&&window.CATTYPES[cat.value])||[]; td.innerHTML=tl.map(function(x){return '<option value="'+h(x)+'">';}).join('');}
    var aw=$('bd_alco_wrap'); if(cat&&aw){aw.style.display=(window.catHasAlcohol&&window.catHasAlcohol(cat.value))?'':'none';}}
