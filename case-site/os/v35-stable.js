(function(){
  'use strict';
  var V=(typeof APP_VERSION!=='undefined'&&APP_VERSION)?String(APP_VERSION):'4.32.6';
  var DRAFT_KEY='asaas-os-brand-draft-v35';
  var BF={q:'',fit:'',chip:'all',view:'table',limit:50,pageSize:50,sort:'',dir:1,filters:{},excel:{}};
  var renderTimer=null, lastFocus=null, BF_USER='', BF_SAVE_T=null, BF_MENU=null;
  ...
  function brandUserToken(){try{var u=(coreS()&&coreS().user)||{};return String(u.id||u.email||u.name||'guest').toLowerCase().replace(/[^a-z0-9_-]+/gi,'_').slice(0,80)||'guest';}catch(e){return 'guest';}}
  function brandPrefKey(){return 'caseos_brand_table_prefs::'+brandUserToken();}
  function ensureBrandPrefs(){var u=brandUserToken();if(BF_USER===u)return;BF_USER=u;var base={q:'',fit:'',chip:'all',view:'table',limit:50,pageSize:50,sort:'',dir:1,filters:{},excel:{}};try{var raw=localStorage.getItem(brandPrefKey()),x=raw?JSON.parse(raw):null;if(x&&typeof x==='object')base=Object.assign(base,x);if(!base.filters||typeof base.filters!=='object')base.filters={};if(!base.excel||typeof base.excel!=='object')base.excel={};}catch(e){}BF=base;}
  function saveBrandPrefs(){ensureBrandPrefs();clearTimeout(BF_SAVE_T);BF_SAVE_T=setTimeout(function(){try{localStorage.setItem(brandPrefKey(),JSON.stringify(BF));}catch(e){}},120);}
  ...
  var CANON_SUBS={
    'F&B / рестораны и кафе':['Ресторан','Кафе','Кофейня','QSR / фастфуд','Фудкорт','Пекарня / выпечка','Десерты / мороженое','Бар / лаундж','Пицца','Бургеры','Национальная кухня','Азиатская кухня','Европейская кухня','Соки / смузи','Чайхана','Шашлычная / гриль','Суши / роллы','Лапшичная','Стритфуд','Шаурма / донер','Кондитерская','Бабл-ти / коктейли','Халяль-ресторан','Банкетный зал','Доставка / dark kitchen','Винотека / винный бар','Кофе с собой','Столовая','Самса / тандыр'],
    'Мода и одежда':['Женская одежда','Мужская одежда','Детская одежда','Обувь','Аксессуары','Нижнее бельё','Масс-маркет','Premium fashion','Одежда multi-brand','Верхняя одежда','Джинсовая одежда','Национальная одежда','Свадебная и вечерняя мода','Сумки и кожгалантерея','Головные уборы','Большие размеры','Униформа / спецодежда','Секонд-хенд / сток','Ткани и фурнитура','Спортивный кэжуал','Чулочно-носочные изделия'],
    'Супермаркет / продукты':['Супермаркет','Гипермаркет','Минимаркет','Продукты','Convenience store','Пекарня-магазин','Мясная лавка','Рыба и морепродукты','Овощи и фрукты','Молочная продукция','Восточные сладости','Орехи и сухофрукты','Здоровое питание / эко','Кулинария','Алкомаркет','Табак / вейп','Вода и напитки','Чай и кофе','Бакалея'],
    'Дом и интерьер':['Мебель','Посуда','Домашний текстиль','Декор и интерьер','Освещение','Товары для дома','Кухни на заказ','Двери и окна','Отделочные материалы','Сантехника','Ковры','Шторы и жалюзи','Постельное бельё','Хозтовары','Инструменты','Садовые товары','Мебельная фурнитура','Матрасы'],
    'Электроника и техника':['Телефоны и гаджеты','Бытовая техника','Компьютеры','Аксессуары для техники','Электроника','Ремонт техники','Аудио и видео','Игровые консоли','Умный дом','Фототехника','Климатическая техника','Электроинструмент','Комплектующие ПК','Сетевое оборудование','Мобильные аксессуары'],
    'Красота и здоровье':['Аптека','Клиника / медцентр','Косметика','Парфюмерия','Салон красоты','Оптика','Здоровье и уход','Барбершоп','Ногтевая студия','Косметология','Массаж / СПА','Солярий','Стоматология','Лаборатория анализов','Медтехника и ортопедия','БАДы и витамины','Тату / пирсинг','Эпиляция','Парикмахерская'],
    'Ювелирные изделия и аксессуары':['Ювелирные изделия','Часы','Бижутерия','Аксессуары','Очки / оптика','Золото','Серебро','Драгоценные камни','Ремонт ювелирных изделий','Ломбард','Часовая мастерская','Свадебные украшения','Кожаные аксессуары'],
    'Спорт и фитнес':['Спортивная одежда','Спорттовары','Фитнес-клуб','Спортивное питание','Экипировка','Тренажёрный зал','Бассейн','Йога / пилатес','Единоборства','Велосипеды','Туризм и активный отдых','Охота и рыбалка','Спортивная обувь','Танцевальная студия','Настольный теннис / бильярд'],
    'Развлечения':['Кинотеатр','Детский развлекательный центр','Игровая зона','Боулинг / бильярд','Аттракционы','VR / киберспорт','Караоке','Квесты','Батутный центр','Ледовый каток','Лазертаг','Детская игровая комната','Антикафе / настольные игры','Концертная площадка','Скалодром','Планетарий / музей'],
    'Дети и образование':['Игрушки','Книги и канцтовары','Детское развитие / образование','Детский центр','Детские товары','Языковая школа','Учебный центр / курсы','IT-школа','Музыкальная школа','Художественная студия','Детский сад / ясли','Робототехника','Репетиторский центр','Школьная форма','Детская обувь','Товары для новорождённых','Детская мебель'],
    'Услуги и сервисы':['Банк / обмен валют','Пункт выдачи','Ремонт / сервис','Ателье','Химчистка','Туризм / авиакасса','Оператор связи / салон связи','Киоск / спец. магазин','Полиграфия / печать','Копицентр','Фотоуслуги / фотостудия','Нотариус','Юридические услуги','Страхование','Микрофинансы / кредиты','Почта / курьерская служба','Ремонт обуви','Ремонт одежды','Ремонт телефонов','Изготовление ключей','Клининг','Прачечная','Свадебное агентство','Кадровое агентство','Рекламное агентство','Логистика / грузоперевозки','Обмен валют','Платёжный терминал','Чистка ковров','Ювелирная мастерская','Гравировка','Наружная реклама'],
    'Специализированный ритейл':['Подарки и сувениры','Оператор / группа','Книги и канцтовары','Товары по одной цене / разное','Специализированный магазин','Зоотовары','Цветы / флористика','Товары для творчества','Музыкальные инструменты','Антиквариат','Товары для праздника','Сувенирная лавка','Религиозные товары','Вейп / кальян','Игры и хобби','Коллекционные товары','Магазин фиксированной цены','Военторг / туризм'],
    'Авто':['Автозапчасти','Автоаксессуары','Автохимия и масла','Автосервис','Автомойка','Шиномонтаж','Автосалон','Прокат авто','Автозвук','Тюнинг','Мото и велотехника','Автострахование','Автостекло','Запчасти для грузовиков'],
    'Офис / коворкинг':['Офис','Коворкинг','Представительство','Бизнес-сервис','Банковский офис','Страховой офис','Медицинский офис','IT-компания','Колл-центр','Переговорные','Туроператор','Государственные услуги','Учебный офис','Нотариальная контора'],
    'Склад / технические помещения':['Склад арендатора','Кладовая','Технический склад','Серверная','Электрощитовая','Санузлы','Коридор / проход','Терраса / летняя зона','Холодильный склад','Разгрузочная зона','Помещение персонала','Комната охраны','Венткамера','Насосная','Лифтовой холл','Паркинг','Мусорная комната','Архив','Пожарный выход'],
    'Прочее':['Прочее','Вакантное помещение','Временная активность (pop-up)','Промо-зона','Рекламная конструкция','Банкомат','Вендинг','Островок / киоск','Не определено']
  };
  var SUB_ALIAS_GROUPS={
    /* v4.50.4: раньше в списке стояло только «Telecom» — единственное английское слово
       среди 32 русских, и смысл размыт: то ли салон связи, то ли интернет-провайдер.
       Сотрудник искал мобильных операторов и не находил. Старое имя оставлено
       псевдонимом, поэтому уже заведённые записи не теряются. */
    'Оператор связи / салон связи':['Telecom','Телеком','Мобильный оператор','Сотовая связь','Салон связи','Оператор связи','Связь','Мобильная связь'],
  ...
  function brandColValue(b,k){if(k==='status')return statusFull(b.status);if(k==='cat')return brandCategory(b);if(k==='sub')return brandSub(b);if(k==='type')return (b.type||'')+(b.alcohol?' (Алкоголь)':'');if(k==='country')return country2(b.country);if(k==='contact')return contactText(b);return b[k]==null?'':b[k];}
  function excelCond(v,f){if(!f)return true;var op=f.op||'',a=String(f.a==null?'':f.a),b=String(f.b==null?'':f.b),sv=String(v==null?'':v),lo=sv.toLowerCase(),aa=a.toLowerCase();if(Array.isArray(f.vals)){var vv=sv||'(Пусто)';if(f.vals.indexOf(vv)<0)return false;}if(!op)return true;if(op==='contains')return lo.indexOf(aa)>=0;if(op==='notcontains')return lo.indexOf(aa)<0;if(op==='starts')return lo.indexOf(aa)===0;if(op==='ends')return aa===''||lo.slice(-aa.length)===aa;if(op==='eqtxt')return lo===aa;if(op==='neqtxt')return lo!==aa;if(op==='empty')return !sv.trim();if(op==='notempty')return !!sv.trim();var n=num(sv),na=num(a),nb=num(b);if(op==='gt')return n>na;if(op==='ge')return n>=na;if(op==='lt')return n<na;if(op==='le')return n<=na;if(op==='eq')return n===na;if(op==='neq')return n!==na;if(op==='between')return n>=Math.min(na,nb)&&n<=Math.max(na,nb);if(op==='notbetween')return n<Math.min(na,nb)||n>Math.max(na,nb);return true;}
  function brandExcelPass(b){var x=BF.excel||{},ks=Object.keys(x);for(var i=0;i<ks.length;i++){if(!excelCond(brandColValue(b,ks[i]),x[ks[i]]))return false;}return true;}
  function passBrand(b){var f=BF.filters||{}; var q=String(BF.q||'').toLowerCase(); if(q){var blob=[b.name,b.cat,b.sub,b.type,(b.alcohol?'алкоголь':''),b.country,b.owner,contactText(b),b.notes,b.reqs,b.format,b.group,b.presence,b.reviewFlag,b.areaVariants,b.about,b.pos,b.concept,b.rec,statusFull(b.status)].join(' ').toLowerCase(); if(blob.indexOf(q)<0)return false;}
    ...
    if(f.cat && !sameCat(b.cat,f.cat))return false; if(f.sub && !textPass(subNorm(b.sub,b.cat)+' '+(b.sub||''),f.sub))return false; if(f.type && !textPass((b.type||'')+(b.alcohol?' алкоголь alcohol':''),f.type))return false; if(f.country && !textPass(country2(b.country)+' '+b.country,f.country))return false; if(f.price && !textPass(b.price,f.price))return false; if(f.tier && !textPass(b.tier,f.tier))return false; if(f.owner && !textPass(b.owner,f.owner))return false; if(f.contact && !textPass(contactText(b),f.contact))return false; if(f.amin && !numPass(b.amin,f.amin))return false; if(f.amax && !numPass(b.amax,f.amax))return false; if(!brandExcelPass(b))return false; return true;}
  ...
  window.asaas35BrandFilter=function(el,key,now){ ensureBrandPrefs();if(!el)return; lastFocus=focusInfo(el); if(key==='q')BF.q=el.value; else if(key==='fit')BF.fit=el.value; else BF.filters[key]=el.value; BF.limit=BF.pageSize||50; if(key==='cat'){var valid=allSubs(el.value); if(BF.filters.sub && valid.length && valid.indexOf(BF.filters.sub)<0) BF.filters.sub='';} saveBrandPrefs();if(now)renderBrandsResultsOnly(el); else scheduleRender(el); };
  window.asaas35Chip=function(chip){ensureBrandPrefs();cancelPendingBrandRender();BF.chip=chip||'all';BF.limit=BF.pageSize||50;saveBrandPrefs();renderBrandsStable();};
  window.asaas35Sort=function(k){ensureBrandPrefs();cancelPendingBrandRender();if(BF.sort!==k){BF.sort=k;BF.dir=1;}else if(BF.dir>0){BF.dir=-1;}else{BF.sort='';BF.dir=1;}saveBrandPrefs();renderBrandsStable();};
  window.asaas35Page=function(mode){ensureBrandPrefs();cancelPendingBrandRender();if(mode==='more')BF.limit=(BF.limit||BF.pageSize||50)+(BF.pageSize||50); else if(mode==='all')BF.limit=999999; else {BF.pageSize=parseInt(mode,10)||50;BF.limit=BF.pageSize;}saveBrandPrefs();renderBrandsStable();};
  window.asaas35ResetFilters=function(){ensureBrandPrefs();cancelPendingBrandRender();var ps=(BF&&BF.pageSize)||50;BF={q:'',fit:'',chip:'all',view:(BF&&BF.view)||'table',limit:ps,pageSize:ps,sort:(BF&&BF.sort)||'',dir:(BF&&BF.dir)||1,filters:{},excel:{}};saveBrandPrefs();renderBrandsStable();};
  window.asaas35ResetSort=function(){ensureBrandPrefs();cancelPendingBrandRender();BF.sort='';BF.dir=1;saveBrandPrefs();renderBrandsStable();};
  window.asaas35ResetAll=function(){ensureBrandPrefs();cancelPendingBrandRender();var ps=(BF&&BF.pageSize)||50;BF={q:'',fit:'',chip:'all',view:(BF&&BF.view)||'table',limit:ps,pageSize:ps,sort:'',dir:1,filters:{},excel:{}};saveBrandPrefs();renderBrandsStable();};
  function sortArrow(k){return BF.sort===k?(BF.dir>0?' <span class="case-grid-sort-on asaas35-sort-arrow">▲</span>':' <span class="case-grid-sort-on asaas35-sort-arrow">▼</span>'):' <span class="sortbtn" title="Можно сортировать">↕</span>';}
  function sortHeader(k,label){var active=!!(BF.excel&&BF.excel[k]);return '<th data-colkey="'+h(k)+'" data-sortable="1" data-filterable="1"><div class="th1r"><span class="th-name" title="1-й клик: по возрастанию; 2-й: по убыванию; 3-й: убрать сортировку" onclick="asaas35Sort(\''+h(k)+'\')">'+h(label)+sortArrow(k)+'</span><span class="tblfnl'+(active?' on':'')+'" title="Фильтр и сортировка" onclick="event.stopPropagation();asaas35ColMenu(\''+h(k)+'\',this)">▾</span></div></th>';}
  function closeBrandMenu(){if(BF_MENU){BF_MENU.remove();BF_MENU=null;}document.removeEventListener('mousedown',brandMenuAway,true);} window.asaas35CloseColMenu=closeBrandMenu;
  if(!window.__asaas35BrandMenuEscape){window.__asaas35BrandMenuEscape=true;document.addEventListener('keydown',function(ev){if(ev.key==='Escape')closeBrandMenu();},true);} 
  function brandMenuAway(ev){if(BF_MENU&&!BF_MENU.contains(ev.target))closeBrandMenu();}
  function brandDistinct(k){var seen={},a=[];brands().forEach(function(b){var v=String(brandColValue(b,k)||'')||'(Пусто)';if(!seen[v]){seen[v]=1;a.push(v);}});return a.sort(function(x,y){return x.localeCompare(y,'ru',{numeric:true,sensitivity:'base'});});}
  window.asaas35ColMenu=function(k,anchor){ensureBrandPrefs();closeBrandMenu();var vals=brandDistinct(k),f=(BF.excel||{})[k]||{},isNum=(k==='amin'||k==='amax'),sel=Array.isArray(f.vals)?f.vals.slice():vals.slice(),ops=isNum?[['','— без условия —'],['gt','Больше (>)'],['ge','Больше или равно (≥)'],['lt','Меньше (<)'],['le','Меньше или равно (≤)'],['eq','Равно (=)'],['neq','Не равно (≠)'],['between','Между'],['notbetween','Вне диапазона'],['empty','Пусто'],['notempty','Не пусто']]:[['','— без условия —'],['contains','Содержит'],['notcontains','Не содержит'],['starts','Начинается с'],['ends','Заканчивается на'],['eqtxt','Равно'],['neqtxt','Не равно'],['empty','Пусто'],['notempty','Не пусто']];var m=document.createElement('div');m.className='case-grid-menu';m.dataset.caseGridOwner='brands';m.innerHTML='<div class="cg-sort"><button data-sort="1">▲ '+(isNum?'По возрастанию':'А → Я')+'</button><button data-sort="-1">▼ '+(isNum?'По убыванию':'Я → А')+'</button></div><div class="cg-title">Условие</div><select data-op style="width:100%;padding:7px;border:1px solid var(--border);border-radius:7px;background:var(--panel);color:var(--ink)">'+ops.map(function(o){return '<option value="'+o[0]+'" '+((f.op||'')===o[0]?'selected':'')+'>'+o[1]+'</option>';}).join('')+'</select><div data-cond style="margin-top:5px"><input type="search" data-a placeholder="'+(isNum?'Значение':'Текст')+'" value="'+h(f.a||'')+'"><input type="search" data-b placeholder="Верхняя граница" value="'+h(f.b||'')+'" style="margin-top:5px;display:'+((f.op==='between'||f.op==='notbetween')?'block':'none')+'"></div><div class="cg-title">Значения</div><input type="search" data-search placeholder="Поиск значения"><div style="font-size:10.5px;margin:5px 0"><a href="#" data-all="1">Выбрать все</a> · <a href="#" data-all="0">Снять все</a></div><div class="cg-values">'+vals.slice(0,700).map(function(v){return '<label><input type="checkbox" data-value="'+h(v)+'" '+(sel.indexOf(v)>=0?'checked':'')+'><span>'+h(v)+'</span></label>';}).join('')+'</div><div class="cg-footer"><button data-reset>Сбросить</button><span class="sp"></span><button data-cancel>Отмена</button><button class="primary" data-apply>Применить</button></div>';document.body.appendChild(m);BF_MENU=m;var r=anchor.getBoundingClientRect(),w=Math.min(310,window.innerWidth-16);m.style.width=w+'px';m.style.left=Math.max(8,Math.min(r.left,window.innerWidth-w-8))+'px';var top=r.bottom+4;if(top+m.offsetHeight>window.innerHeight-8)top=Math.max(8,window.innerHeight-m.offsetHeight-8);m.style.top=top+'px';m.querySelectorAll('[data-sort]').forEach(function(b){b.onclick=function(){BF.sort=k;BF.dir=+b.dataset.sort;saveBrandPrefs();closeBrandMenu();renderBrandsStable();};});var op=m.querySelector('[data-op]'),cond=m.querySelector('[data-cond]'),bIn=m.querySelector('[data-b]');op.onchange=function(){var v=op.value;cond.style.display=(v==='empty'||v==='notempty')?'none':'block';bIn.style.display=(v==='between'||v==='notbetween')?'block':'none';};op.onchange();var search=m.querySelector('[data-search]');search.oninput=function(){var q=search.value.toLowerCase();m.querySelectorAll('.cg-values label').forEach(function(l){l.style.display=l.textContent.toLowerCase().indexOf(q)>=0?'flex':'none';});};m.querySelectorAll('[data-all]').forEach(function(a){a.onclick=function(ev){ev.preventDefault();var on=a.dataset.all==='1';m.querySelectorAll('.cg-values label').forEach(function(l){if(l.style.display!=='none')l.querySelector('input').checked=on;});};});m.querySelector('[data-reset]').onclick=function(){delete BF.excel[k];saveBrandPrefs();closeBrandMenu();renderBrandsStable();};m.querySelector('[data-cancel]').onclick=closeBrandMenu;m.querySelector('[data-apply]').onclick=function(){var chosen=[],o=op.value,a=m.querySelector('[data-a]').value.trim(),bb=bIn.value.trim();m.querySelectorAll('.cg-values input:checked').forEach(function(c){chosen.push(c.dataset.value);});if(chosen.length===vals.length&&!o)delete BF.excel[k];else{BF.excel[k]={};if(chosen.length!==vals.length)BF.excel[k].vals=chosen;if(o){BF.excel[k].op=o;if(o!=='empty'&&o!=='notempty')BF.excel[k].a=a;if(o==='between'||o==='notbetween')BF.excel[k].b=bb;}}saveBrandPrefs();closeBrandMenu();renderBrandsStable();};setTimeout(function(){document.addEventListener('mousedown',brandMenuAway,true);},0);};
  window.asaas35SetView=function(v){ensureBrandPrefs();cancelPendingBrandRender();BF.view=v||'table';saveBrandPrefs();renderBrandsStable();};
  ...
  function buildBrandTbody(shown){
    ...
    var rowHtml=shown.map(function(b){var cat=brandCategory(b), sub=brandSub(b), typ=(b.type||'')+(b.alcohol?' (Алкоголь)':''), cont=contactText(b)||'-'; return '<tr onclick="'+open+'(\''+h(b.id)+'\')" title="Открыть карточку бренда">'+
      ...
      '<td class="type" title="'+h(typ)+'">'+h(typ||'-')+'</td>'+
      ...
      '</tr>';}).join('');
    return rowHtml||'<tr><td colspan="15" class="empty">Нет записей по фильтрам</td></tr>';
  }
  function buildBrandContent(shown){
    ...
    function brandCard(b){var cont=contactText(b)||'-'; return '<div class="asaas35-card" onclick="'+open+'(\''+h(b.id)+'\')"><div class="badge">'+h(statusShort(b.status))+'</div><h3>'+h(b.name||'-')+(b.reviewFlag?' <span title="'+h(b.reviewFlag)+'" style="color:#d17b00">⚠</span>':'')+'</h3><p>'+h(b.cat||'-')+' / '+h(b.sub||'-')+(b.type?' · '+h(b.type)+(b.alcohol?' (Алк.)':''):'')+'</p><p>'+h(country2(b.country))+' · '+h((b.amin||'-')+'–'+(b.amax||'-'))+' м²</p><small>'+h(cont)+'</small></div>';}
    ...
    /* v4.70.2: tbl-scroll рядом со своей обёрткой. Табличное ядро ищет прокрутку именно по
       этому классу, и без него База брендов оставалась единственной таблицей без панели
       «Столбцы / Вид», без закрепления столбцов и без верхней полосы прокрутки. */
    return '<div class="asaas35-table-wrap tbl-scroll"><table class="asaas35-table" data-case-grid-key="brands" data-grid-engine="brands"><thead><tr>'+ 
      '<th class="sel" data-colkey="select"></th>'+sortHeader('name','Название')+sortHeader('status','Статус')+sortHeader('cat','Категория')+sortHeader('sub','Подкатегория')+sortHeader('type','Тип')+sortHeader('country','Страна')+sortHeader('price','Цена, уровень')+sortHeader('tier','Бренд, уровень')+sortHeader('amin','Мин. площадь, м²')+sortHeader('amax','Макс. площадь, м²')+sortHeader('owner','Ответственный')+sortHeader('contact','Контакт')+'<th data-colkey="actions">Действия</th><th data-colkey="notes">Комментарии</th></tr>'+ 
      '<tr class="filters"><th></th><th>'+filterInput('bf_name','q','фильтр...','')+'</th><th>'+filterInput('bf_status','status','Все','dl_status')+'</th><th>'+filterInput('bf_cat','cat','Все','dl_cat')+'</th><th>'+filterInput('bf_sub','sub','фильтр...','dl_sub')+'</th><th>'+filterInput('bf_type','type','фильтр...','dl_type')+'</th><th>'+filterInput('bf_country','country','Все','dl_country')+'</th><th>'+filterInput('bf_price','price','A-D','dl_level')+'</th><th>'+filterInput('bf_tier','tier','A-D','dl_level')+'</th><th>'+filterInput('bf_amin','amin','>=50','')+'</th><th>'+filterInput('bf_amax','amax','<=300','')+'</th><th>'+filterInput('bf_owner','owner','фильтр...','dl_owner')+'</th><th>'+filterInput('bf_contact','contact','фильтр...','dl_contact')+'</th><th></th><th></th></tr></thead><tbody>'+buildBrandTbody(shown)+'</tbody></table></div>';
  }
  function brandRenderState(){ensureBrandPrefs();
    var all=brands();
  ...
  function renderBrandsStable(){var st=brandRenderState(); var all=st.all, rows=st.rows, shown=st.shown; var subList=allSubs(BF.filters.cat); var catsList=cats(); var statusList=Object.keys(statuses()).map(statusFull); var countries=uniq(all.map(function(b){return country2(b.country);})).sort(); var owners=uniq(all.map(function(b){return b.owner||'';})).sort(); var contacts=uniq(all.map(contactText)).slice(0,500); var chip=function(id,label){return '<button class="asaas35-chip '+(BF.chip===id?'on':'')+'" onclick="asaas35Chip(\''+id+'\')">'+h(label)+'</button>';};
    ...
    var html='<section class="asaas35-head ph" data-asaas35-brand-shell="1"><div><div class="eye">БАЗА БРЕНДОВ</div><h1>Бренды</h1><p>Excel-таблица с фильтрами, быстрым вводом и стабильным черновиком карточки.</p></div><div class="asaas35-stats"><span>КД</span><b id="asaas35-count">'+shown.length+' / '+rows.length+' / '+all.length+'</b></div></section>'+ 
  ...
  function refreshBrandSubDatalist(){var cat=$('bd_cat'), sub=$('bd_sub'), dl=$('brandSubDL'); if(cat&&dl){var list=allSubs(cat.value); dl.innerHTML=list.map(function(x){return '<option value="'+h(x)+'">';}).join(''); if(sub && sub.value && list.length && list.indexOf(sub.value)<0){sub.title='Эта подкатегория не входит в выбранную категорию, но может быть сохранена вручную.';}}
    /* v4.50.4: типы лежат под КАНОНИЧЕСКИМИ названиями категорий, а в поле может быть
  ...
  function brandFormStable(b,isNew){var d=b._draft||{}; var sts=Object.keys(statuses()).map(function(k){return [k,statusFull(k)];}); var presenceOpts=[['','—'],['Есть в Узбекистане','Есть в Узбекистане'],['Нет в Узбекистане','Нет в Узбекистане'],['Выходит на рынок','Выходит на рынок'],['Уточнить','Уточнить']]; var levels=[['','—'],['A','A'],['B','B'],['C','C'],['D','D']]; var catList=cats(); var currentCat=(d.bd_cat!==undefined?d.bd_cat:catNorm(b.cat)); var currentSub=(d.bd_sub!==undefined?d.bd_sub:subNorm(b.sub,b.cat)); var subList=allSubs(currentCat); var typeList=(window.CATTYPES&&window.CATTYPES[currentCat])||[]; var contacts=contactListFromBrand(b,d); var html='<div class="dh"><span class="x" onclick="closeDrawer()">×</span><h2>'+(isNew?'Новый бренд — черновик':'Редактирование бренда')+'</h2><div class="s">Поля сохраняются локально. В базу попадает только после “Сохранить”.</div></div><div class="dbody asaas35-brand-form" data-brand-draft="'+(isNew?'new':'edit')+'">'+
      ...
      '<div class="grid2">'+input('Тип','bd_type',(d.bd_type!==undefined?d.bd_type:(b.type||'')),'brandTypeDL')+'<div id="bd_alco_wrap" style="'+((window.catHasAlcohol&&window.catHasAlcohol(currentCat))?'':'display:none')+'"><label><span>Алкоголь</span><span style="display:flex;align-items:center;gap:8px;padding-top:6px"><input type="checkbox" id="bd_alco" style="width:auto"'+(((d.bd_alco!==undefined?d.bd_alco:b.alcohol))?' checked':'')+'> продаёт / подаёт алкоголь</span></label></div>'+'</div>'+
