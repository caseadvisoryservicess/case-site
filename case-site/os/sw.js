...
self.addEventListener('install', event => {
  ...
  event.waitUntil(caches.open(CACHE).then(cache => {
    const failed = [];
    return Promise.all(ASSETS.map(u => cache.add(u).catch(() => { failed.push(u); })))
      .then(() => { if (failed.length) console.warn('[CASE OS sw] не удалось закэшировать:', failed); });
  }).catch(e => console.warn('[CASE OS sw] предзагрузка не выполнена:', e)));
  self.skipWaiting();
});
