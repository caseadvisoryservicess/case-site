/* CASE OS v4.17.0 — Мастер-модуль «Планировки и LCR» (#64, ядро аудита).
...
(function(){
  ...
  function renderOverview(pid){
    ...
    return card('<h3 style="margin:0 0 6px">'+h(o.name||pid)+' — Планировки и LCR</h3><p style="font-size:12.5px;color:var(--muted);margin:0 0 10px">Единый контур: версии → интерактивный план → блоки/этажи → юниты → валидация → генерация LCR → передача аренде. Все вкладки используют одни и те же записи проекта.</p><div style="display:flex;gap:14px;flex-wrap:wrap">'+kpis.map(function(k){return '<div style="min-width:110px"><div style="font-size:22px;font-weight:700">'+h(k[1])+'</div><div style="font-size:11px;color:var(--muted)">'+h(k[0])+'</div></div>';}).join('')+'</div>')
      +card('<b>Быстрые действия</b><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px">'+link('plans','Интерактивный план')+link('registry','Реестр помещений (LCR)')+link('leasing_layouts','Реестр версий (файлы)')+'<button class="btn sm" onclick="masterTab(\'lcr\')">Генерация LCR</button></div>')
