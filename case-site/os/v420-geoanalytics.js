...
(function(){
  ...
  function context(){
    ...
    fr.contentWindow.postMessage({source:'asaas-os-v4',type:'asaas-geo-context',context:{
      ...
      /* v4.73.1: проекты CASE в студию не передаются (projects пустой): клиентам они не нужны.
         Сохранённые гео-профили проектов по-прежнему едут в geoData и возвращаются при
         сохранении нетронутыми. */
      editable:canEdit(),adminEdit:(function(){try{var r=typeof R==='function'?R():{};return !!(r.geoEdit||r.admin);}catch(e){return false;}})(),external:isExternal(),user:(S&&S.user)||null,projects:[],activeProjectId:'',probeProjectId:'',geoData:(typeof GEO_DATA!=='undefined'?GEO_DATA:null),geoRevision:G.geoRevision||0
    }},location.origin);
  }
  ...
  function css(){
    ...
    var s=document.createElement('style');s.id='geoV42Css';s.textContent=''
      +'.main.geo-workspace{padding:7px 9px 0}.geo-v42{min-width:0}.geo-v42-toolbar{display:flex;gap:6px;align-items:center;background:var(--panel);border:1px solid var(--border);border-radius:10px 10px 0 0;padding:6px}.geo-v42-title{flex:1;min-width:0;padding:7px 9px;font:600 11.5px inherit;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.geo-v42-recover{border:1px solid #d8bd76;border-radius:7px;background:#fff8e8;color:#7a5200;padding:7px 10px;font:800 10.5px inherit;cursor:pointer;white-space:nowrap}.geo-v42-recover:hover{background:#f7e8be}.geo-v42-hist{border:1px solid var(--border);border-radius:7px;background:var(--panel);color:var(--ink);padding:7px 11px;font:800 10.5px inherit;cursor:pointer;white-space:nowrap}.geo-v42-hist:hover{background:var(--soft);color:var(--red-d)}.geo-v42-reload{flex:0 0 34px;width:34px;height:32px;border:1px solid var(--border);border-radius:7px;background:var(--panel);color:var(--ink);font:800 16px inherit}.geo-v42-reload:hover{background:var(--soft);color:var(--red-d)}.geo-v42-frame{position:relative;border:1px solid var(--border);border-top:0;border-radius:0 0 10px 10px;overflow:hidden;background:#f4f3f1;box-shadow:var(--shadow)}.geo-v42-frame iframe{display:block;border:0;width:100%;height:calc(100dvh - 105px);min-height:480px;background:#f4f3f1}.geo-v42-loader{position:absolute;inset:0;z-index:2;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.86);font-size:12px;font-weight:800;color:var(--red-d)}'
