...
| §10 | Switchable base maps | BUILT | `src/js/10b-basemaps.js` is the registry; 11 checks in `qa`. Six providers ship active (OpenStreetMap, OSM Humanitarian, CARTO Positron, CARTO Dark Matter, Esri imagery, OpenTopoMap) plus an explicit "no base map" grid. Every active provider carries its attribution, asserted rather than trusted. |
| §10 | 2GIS / Google / Yandex base maps | **LICENSED, NOT SHIPPED** | Present in the switcher, disabled, each naming the one thing that unlocks it and offering a field for the licensed endpoint. All three forbid direct tile access: Google only through the Maps JavaScript API with a billing-enabled key, Yandex only through its own API, 2GIS only under a licence agreement. The unofficial XYZ endpoints that circulate are those same terms, broken. Yandex additionally draws in EPSG:3395 against this map's EPSG:3857, which without reprojection puts its tiles a few hundred metres off our markers – the entry says so. |
...
Base maps are now switchable (six providers plus a grid), which §10 asked for. What is still
declined there is narrower than it was: the **context layers**. A metro layer needs verified
