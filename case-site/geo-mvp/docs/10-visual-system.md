...
## 12. Base maps and dark tiles

`src/js/10b-basemaps.js` holds every background the map can draw. Two of the registry's rules are
visual:
...
- Switching **crossfades**. The outgoing layer is held until the incoming one paints, or until its
  ...
  without the error path the old basemap would sit on top of the new one for the whole timeout.
